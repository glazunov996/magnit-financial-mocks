<script>
  ...
  import { checkArrowDynamics, renderDynamicsClass } from 'utilities/indicator';
  ...
  $: calculateValues(data, bridge, baseRatio, $units);
    ...
    {#if data}
      <svg
        ...
        height="19">
          <rect
            ...
            {#if bridge.qNum !== 0}
              <rect
                class={renderDynamicsClass('txcm-factorsTotalDiagramRectBase', checkArrowDynamics(isInverted, bridge.qNum))}
                {...RECT}
      ...
      <div
        class="txcm-factorsTotalDiagramDiff">
          {formatLabel(bridge.qText, $units)}
      </div>
    {/if}
</div>
