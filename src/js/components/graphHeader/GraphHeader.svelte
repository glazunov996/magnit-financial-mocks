<script>
  ...
  function generateWeekFragment() {    
    ...
    console.log("GENERATE WEEK FRAGMENT", datePoP, $step, weekStart); // 1583107200000 : 02.03.2021 // 1556668800000
    const result = [];
    for (let index = 0; index <= $count; index += 1) {
      const week = (weekStart + $step + index) % (SCALE_WEEKS_COUNT + 1) + 1;
      const label = `${week}`;
      result.push({ label });
    }
    return result;
