<script>
  ...
  $: filtered = filterNavigationCategories(categories, filter, $location);
    ...
    {filter}>
      {#each group.indicators as indicator}
        <NavLink
          ...
          to={!$location.endsWith('factors') || indicator.id === '7591' || indicator.id === '7661' || indicator.id === '7492' || indicator.id === '7589'
